// Usually, this source file contains the implementation for our code that's declared in the header.
// In our case however the class is a template and everything is in the header file.
// We'll use this file to store additional tests so they will be run when building the project.

#include "circular_buffer.h"
#ifdef EXERCISM_TEST_SUITE
// Note: in the tests that come with the exercise, this include is actually
//   #include <catch2/catch.hpp>
// but the C++ test runner replaces that with the value below, which we also
// need to use if we want our custom tests to run on the Exercism platform.
