// This file contains additional tests for features of the buffer class that
// are not tested in the tests that come with the exercise. I haven't found
// a proper way to have them executed on the Exercism infrastructure, but
// I'll submit them with the exercise so they can be run locally if need be.
...
TEST_CASE("test_with_move_only_type") 
{
    ...
    struct foo {
        int i_;
        explicit foo(int i) noexcept : i_{i} {}
...
TEST_CASE("test_variadic_write")
{
    ...
    struct foo {
        int i_, j_;
        foo(int i, int j) noexcept : i_{i}, j_{j} {}
    ...
    struct bar : foo {
        bar(int i, int j) noexcept : foo{i, j} {}
