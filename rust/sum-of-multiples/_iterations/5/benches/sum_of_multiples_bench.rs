...
fn from_factors_benchmark(c: &mut Criterion) {
    c.bench_function(
        ...
        |b| b.iter(|| sum_of_multiples_from_factors(black_box(10000), black_box(&[43, 47]))),
    );
    c.bench_function(
        "from_factors: solutions_using_include_exclude_must_extend_to_cardinality_greater_than_3",
        |b| b.iter(|| sum_of_multiples_from_factors(black_box(10000), black_box(&[2, 3, 5, 7, 11]))),
    );
}
...
fn from_range_benchmark(c: &mut Criterion) {
    c.bench_function(
        ...
        |b| b.iter(|| sum_of_multiples_from_range(black_box(10000), black_box(&[43, 47]))),
    );
    c.bench_function(
