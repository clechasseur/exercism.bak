...
impl<'a> Hand<'a> {
    pub fn new(hand_s: &'a str) -> Result<Hand<'a>, Error> {
        let mut cards = hand_s.split_whitespace()
            .map(Card::new)
            .collect::<Result<Vec<_>, _>>()
            .and_then(|v| if v.len() == 5 { Ok(v) } else {
                Err(Error::InvalidCardCount(v.len()))
            })?;
        cards.sort_unstable();
        ...
        if Self::is_straight(cards) {
            ...
            if cards[1] == cards[2] {
                contributing_cards = Some(cards[0..3].into());
                non_contributing_cards = Some(cards[3..].into());
            } else {
                contributing_cards = Some(cards[2..].into());
                non_contributing_cards = Some(cards[0..2].into());
            }
        } else {
